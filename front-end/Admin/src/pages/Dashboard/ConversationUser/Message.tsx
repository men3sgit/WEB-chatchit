        ...
        if (hasText) {
            ...
            return parts.map((part, index) => {
                if (part.match(urlRegex)) {
                    // If part is a URL
                    const url = part.startsWith("http") ? part : `http://${part}`; // Add http:// if missing
                    return (
                    ...
                    // If part is regular text
                    // Check if part contains potential domain (e.g., facebook.com) and convert it to a link
                    const domainRegex = /(\b(?:https?:\/\/)?(?:www\.)?[-a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z]{2,6}\b[-a-zA-Z0-9@:%_\+.~#?&//=]*)/gi;
                    const replacedText = part.replace(domainRegex, (match) => {
                        const url = match.startsWith("http") ? match : `http://${match}`;
                        return `<a href="${url}" target="_blank" rel="noopener noreferrer">${match}</a>`;
                    });

                    return (
                        <span key={index} className="ctext-content" dangerouslySetInnerHTML={{ __html: replacedText }} />
                    );
                }
            });
        }
            ...
            <div className="conversation-list">
              ...
              Forwarded
                    ...
                    <div className="ctext-wrap">
                        ...
                        {hasImages ? (
                            <>
                                <Images images={message.image!} message={message} onSetReplyData={onSetReplyData}
                                        onDeleteImg={onDeleteImg}/>
                            </>
                        ) : (
                            <>
                                <div className="ctext-wrap-content">
                                    ...
                                    {hasText && (
                                        <>
                                            ...
                                            {!renderMessageContent() && <p className="mb-0 ctext-content">{message.text}</p>}
                                        </>
                                    )}
